 ...
 * Module: sine_gen_440
 * Project: DVS_Basys3
 * Description: Direct Digital Synthesizer (DDS) generating a clean 440 Hz (A4 concert pitch)
 *              sine wave at 44.1 kHz sample rate for testing the DAC audio output stage.
 */

module sine_gen_440 (
    input  logic        clk,      // 100 MHz System Clock
    input  logic        rst,
    output logic [15:0] sine_out  // 16-bit signed PCM audio output
);
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (sample_clk_cnt >= 18'd2267) begin // ~44.1 kHz sampling rate
                sample_clk_cnt <= 0;
                phase_acc      <= phase_acc + 32'd42852281; // 440 Hz step: (440 * 2^32) / 44100
